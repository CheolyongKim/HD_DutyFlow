...
public class OrderService {
    ...
    private DutyCheckResult checkDutyFreeLimits(List<OrderDTO> items,
        ...
        if (alcoholReg != null && totalAlcohol > alcoholReg.getLimitCapacity()) {
            exceeded = true;
            message.append("- 주류 면세 한도(")
                   .append(alcoholReg.getLimitCapacity())
                   .append("ml)를 초과했습니다.\n");
        }

        if (perfumeReg != null && totalPerfume > perfumeReg.getLimitCapacity()) {
            exceeded = true;
            message.append("- 향수/화장품 면세 한도(")
                   .append(perfumeReg.getLimitCapacity())
                   .append("ml)를 초과했습니다.\n");
        }
    ...
    /**
     * 세금 계산 - 전략 패턴 적용
     * HeejinMain의 calculate()와 동일한 방식
     */
    private BigDecimal calculateTax(List<OrderDTO> items,
                                     RegulationDTO generalReg,
                                     RegulationDTO alcoholReg,
                                     RegulationDTO perfumeReg) {
    ...
    private Order buildOrderForTax(List<OrderDTO> items) {
        ...
        for (OrderDTO item : items) {
            ...
            BigDecimal itemPrice = item.getDollarPrice()
